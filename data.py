# data.py — MMJ Emirates Cup — Teams, Players, Tournament Structure
...
def get_by_pos(pos):
    """1-indexed position"""
    return get_team(TEAM_ORDER[pos - 1])

def get_team_name(code):
    return TEAMS[code]["name"]

def get_all_players():
    """Returns list of {player, team_code, team_name}"""
    result = []
    for code, t in TEAMS.items():
        for p in t["players"]:
            result.append({"player": p, "team_code": code, "team_name": t["name"]})
    return result
